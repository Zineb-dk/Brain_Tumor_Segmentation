...
import tempfile
import os
import ants  # Added ANTs for brain mask generation
...
def get_bounding_box(mask):
    ...
    if len(indices[0]) == 0:  # If mask is empty
        return None
...
def resize_volume(img, target_shape=(144, 144, 144)):
    """Resize a 3D volume to target shape using scipy."""
    ...
    # Use scipy's zoom function which properly handles 3D volumes
    resized = ndimage.zoom(img, zoom_factors, order=1, mode='nearest')
...
def preprocess_image(image_paths):
    ...
    for img_path in image_paths:
        ...
        try:
            # Convert to ANTs image for mask generation
            ants_img = ants.from_numpy(img_data, origin=None, spacing=None, direction=None)
            
            # Create a mask - ANTs automatically generates a brain mask
            mask = ants.get_mask(ants_img)
            mask_data = mask.numpy()
            
            # Get bounding box from mask
            bbox = get_bounding_box(mask_data)
            
            # If no valid bounding box found, use original cropping method
            if bbox is None:
                cropped_img = img_data[48:192, 48:192, 5:149]  # Fallback to fixed cropping
            else:
                min_x, max_x, min_y, max_y, min_z, max_z = bbox
                # Add padding to avoid cutting off brain edges
                padding = 5
                min_x = max(0, min_x - padding)
                min_y = max(0, min_y - padding)
                min_z = max(0, min_z - padding)
                max_x = min(img_data.shape[0] - 1, max_x + padding)
                max_y = min(img_data.shape[1] - 1, max_y + padding)
                max_z = min(img_data.shape[2] - 1, max_z + padding)
                
                # Crop image to bounding box
                cropped_img = img_data[min_x:max_x+1, min_y:max_y+1, min_z:max_z+1]
                
                # Always resize to exactly 144x144x144
                cropped_img = resize_volume(cropped_img, (144, 144, 144))
        except Exception as e:
            # Fallback to original cropping in case of any issues
            st.warning(f"Intelligent cropping failed for {os.path.basename(img_path)}, using fixed cropping instead. Error: {str(e)}")
            cropped_img = img_data[48:192, 48:192, 5:149]  # Fixed cropping
    ...
    if st.button("Process Images and Predict"):
        if flair_file and t1ce_file and t2_file:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                try:
                    with st.spinner('Preprocessing images (creating brain mask and intelligent cropping)...'):
